...
class TestCommentChangesetAPIView(APITestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            username="test_2", password="password", email="a@a.com"
        )
        UserSocialAuth.objects.create(
            ...
            extra_data={
                "id": "123123",
                "access_token": {"oauth_token": "aaaa", "oauth_token_secret": "bbbb"},
            },
        )
    ...
    def test_comment_good_changeset(self, mock_oauth_client):
        ...
        message = """Hello! Awesome edit! & :~) óã
            ...
            """
        response = self.client.post(
            reverse("changeset:comment", args=[self.good_changeset.id]), data=comment
        )
    ...
    def test_comment_unreviewed_changeset(self, mock_oauth_client):
        ...
        message = """Hello! Do you know this area?
            ...
            """
        response = self.client.post(
            reverse("changeset:comment", args=[self.changeset.id]), data=comment
        )
    ...
    def test_comment_changeset_doesnt_exist(self):
        ...
        comment = {"message": "Hello! Awesome edit"}
        response = self.client.post(
            reverse("changeset:comment", args=[2323]), data=comment
        )

        self.assertEqual(response.status_code, 404)


# changeset API is offline, tests commented for
# re-use when re-implementing
...
#         self.client.login(username=self.user.username, password="password")
#         response = self.client.get(reverse("changeset:comment", args=[1234]))
#         self.assertEqual(response.status_code, 404)
#
#     def test_changeset_without_comments(self):
#         self.client.login(username=self.user.username, password="password")
#         response = self.client.get(reverse("changeset:comment", args=[1343]))
#         self.assertEqual(response.status_code, 200)
