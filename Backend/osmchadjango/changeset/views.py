...
class CheckChangeset(ModelViewSet):
    ...
    def update_changeset(self, changeset, request, harmful):
        ...
        return Response(
            {
                "detail": "Changeset marked as {}.".format(
                    "harmful" if harmful else "good"
                )
            },
            status=status.HTTP_200_OK,
        )
...
class ChangesetCommentAPIView(ModelViewSet):
    ...
    def get_comments(self, request, pk):
        ...
        if data.status_code != 200:
            ...
            return Response(
                data.json().get("properties").get("comments"), status=status.HTTP_200_OK
            )
    ...
    def add_footer(self, message):
        ...
        return """{}
            ...
            """.format(
            message, status, self.changeset.id
        )
...
def add_feature(request):
    ...
    feature["url"] = "{}-{}".format(feature.get("osm_type"), feature.get("osm_id"))
    ...
    changeset.save(update_fields=changeset_fields_to_update)
    print("Changeset {} {}".format(changeset.id, "created" if created else "updated"))
    add_reasons_to_changeset(changeset, reasons)
    return Response(
        {"detail": "Feature added to changeset."}, status=status.HTTP_200_OK
    )
