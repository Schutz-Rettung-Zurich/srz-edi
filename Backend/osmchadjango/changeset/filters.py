...
class ChangesetFilter(GeoFilterSet):
    ...
    objectTagArea = GeometryFilter(
        field_name='bbox',
        lookup_expr='intersects',
        help_text="""Filter changesets by object Tags that have changed."""
    )
    ...
    filterMember = filters.CharFilter(
        field_name='filterMember',
        method='filter_filterMember',
        help_text="""Ignore Changesets from Members of the filter"""
    )
    usersToTrack = filters.CharFilter(
        field_name='usersToTrack',
        method='filter_usersToTrack',
        help_text="""Filter Changesets from Users to track"""
    )

    def filter_usersToTrack(self, queryset, name, value):
        usersToTrack = self.data['usersToTrack']
        splittedUsers = usersToTrack.split(',')
        splittedStrippedUser = []
        for user in splittedUsers:
            splittedStrippedUser.append(user.strip())

        return queryset.filter(user__in=splittedStrippedUser)

    def filter_filterMember(self, queryset, name, value):
        usersList = []
        if 'usersToTrack' in self.data:
            users = self.data['usersToTrack']
            splittedUsers = users.split(',')
            for user in splittedUsers:
                usersList.append(user.strip())
        filterMembers = self.data['filterMember']
        splittedMember = filterMembers.split(',')
        for member in splittedMember:
            member = member.strip()
            if member not in usersList:
                queryset = queryset.exclude(user=member)
    ...
    class Meta:
        model = Changeset
        fields = ['geometry', 'objectTagArea', 'users', 'area_lt']
