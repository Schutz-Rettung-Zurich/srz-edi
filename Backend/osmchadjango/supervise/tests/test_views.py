...
from ...changeset.tests.modelfactories import (
    ...
    SuspicionReasonsFactory,
    TagFactory,
    UserWhitelistFactory,
)
...
class TestAoIListView(APITestCase):
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            username="test_user", email="b@a.com", password="password"
        )
        ...
        self.user_2 = User.objects.create_user(
            username="test", email="b@a.com", password="password"
        )
        UserSocialAuth.objects.create(
            user=self.user_2,
            provider="openstreetmap",
    ...
    def test_list_view_released(self):
        self.client.login(username=self.user_2.username, password="password")
        response = self.client.get(reverse("supervise:aoi-get-released"))
        self.assertEqual(response.status_code, 200)
...
class TestAoICreateView(APITestCase):
    def setUp(self):
        ...
        self.data = {
            "name": "Golfo da Guiné",
            "filters": {"is_suspect": "True", "geometry": self.polygon.geojson},
        }
        self.data_bbox = {
            "name": "Golfo da Guiné",
            "filters": {"is_suspect": "True", "in_bbox": "2,0,5,2"},
        }
        self.without_geo_aoi = {
            "name": "Unchecked suspect changesets",
            "filters": {"is_suspect": "True", "checked": "False"},
        }
    ...
    def test_create_AOI(self):
        ...
        self.assertTrue(
            aoi.geometry.intersects(Polygon([[2, 0], [5, 0], [5, 2], [2, 2], [2, 0]]))
        )
    ...
    def test_auto_user_field(self):
        user_2 = User.objects.create_user(
            username="test", email="c@a.com", password="password"
        )
        UserSocialAuth.objects.create(
            user=user_2,
            provider="openstreetmap",
...
class TestAoIStatusAPIViews(APITestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            username="test_user", email="b@a.com", password="password"
        )
        ...
        self.url = reverse("supervise:aoi-status", args=[self.area.pk])
        self.data = "finished=123"

    def test_set_status_unauthenticated(self):
    ...
    def test_set_status(self):
        ...
        response = self.client.patch(self.url, self.data)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data, {"finished": ["123"]})
    ...
    def test_set_get_status_different_user(self):
        self.client.login(username=self.user.username, password="password")
        self.client.patch(self.url, "inProgress=2")
        self.client.patch(self.url, "inProgress=3")
        self.client.patch(self.url, "inProgress=4")
        self.client.login(username=self.user_2.username, password="password")
        response = self.client.get(self.url)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data, {"inProgress": ["2", "3", "4"]})
        self.client.patch(self.url, "finished=3")
        response = self.client.get(self.url)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data, {"inProgress": ["2", "4"], "finished": ["3"]})

    def test_set_status_wrong_format(self):
        self.client.login(username=self.user.username, password="password")
        response = self.client.patch(self.url, "inPorgess=123")
        self.assertEqual(response.status_code, 403)
        self.assertEqual(response.data, {"detail": "Error. Wrong format"})
...
class TestAoIDetailAPIViews(APITestCase):
    ...
    def test_retrieve_detail_authenticated(self):
        ...
        response = self.client.get(reverse("supervise:aoi-detail", args=[self.aoi.pk]))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data["properties"]["name"], "Best place in the world")
        self.assertEqual(
        ...
        self.assertNotIn("user", response.data.keys())
        self.assertEqual(
    ...
    def test_update_aoi_unauthenticated(self):
        ...
        response = self.client.put(
            reverse("supervise:aoi-detail", args=[self.aoi.pk]), self.data
        )
        self.assertEqual(response.status_code, 401)
    ...
    def test_update_aoi_of_another_user(self):
        ...
        user = User.objects.create_user(
            username="test_2", email="c@a.com", password="password"
        )
    ...
    def test_update_with_aoi_owner_user(self):
        ...
        self.assertTrue(
            self.aoi.geometry.intersects(Polygon(((4, 0), (5, 0), (5, 1), (4, 0))))
        )

    def test_put_update_with_bbox(self):
        """'in_bbox' field must populate the geometry field with a Polygon"""
        data = {
            "filters": {"is_suspect": "True", "in_bbox": "4,0,5,1"},
            "name": "Golfo da Guiné",
        }
        self.client.login(username=self.user.username, password="password")
        response = self.client.put(
            reverse("supervise:aoi-detail", args=[self.aoi.pk]), data
        )
        self.assertEqual(response.status_code, 200)
        self.aoi.refresh_from_db()
        self.assertEqual(self.aoi.name, "Golfo da Guiné")
    ...
    def test_patch_update_with_bbox(self):
        """'in_bbox' field must populate the geometry field with a Polygon"""
        data = {"filters": {"is_suspect": "True", "in_bbox": "4,0,5,1"}}
        self.client.login(username=self.user.username, password="password")
        response = self.client.patch(
            reverse("supervise:aoi-detail", args=[self.aoi.pk]), data
        )
...
class TestAoIChangesetListView(APITestCase):
    ...
    def test_aoi_with_in_bbox_filter(self):
        aoi_with_in_bbox = AreaOfInterest.objects.create(
            ...
            filters={"editor": "Potlatch 2", "harmful": "False", "in_bbox": "0,0,2,2"},
        )
        ChangesetFactory(
            harmful=False, bbox=Polygon(((10, 10), (10, 11), (11, 11), (10, 10)))
        )
        ChangesetFactory(
            editor="JOSM 1.5",
            harmful=False,
    ...
    def test_aoi_with_hide_whitelist_filter(self):
        aoi = AreaOfInterest.objects.create(
            name="Another place in the world",
            user=self.user,
            filters={"editor": "Potlatch 2", "hide_whitelist": "True"},
        )
        user_2 = User.objects.create_user(
            username="user_2", email="b@a.com", password="password"
        )
        UserSocialAuth.objects.create(
            user=user_2,
            provider="openstreetmap",
    ...
    def test_aoi_with_false_hide_whitelist_filter(self):
        aoi = AreaOfInterest.objects.create(
            name="Another place in the world",
            user=self.user,
            filters={"editor": "Potlatch 2", "hide_whitelist": "False"},
        )
        user_2 = User.objects.create_user(
            username="user_2", email="b@a.com", password="password"
        )
    ...
    def test_aoi_with_blacklist_filter(self):
        aoi = AreaOfInterest.objects.create(
            name="Another place in the world",
            user=self.user,
            filters={"editor": "Potlatch 2", "blacklist": "True"},
        )
    ...
    def test_aoi_with_false_blacklist_filter(self):
        aoi = AreaOfInterest.objects.create(
            name="Another place in the world",
            user=self.user,
            filters={"editor": "Potlatch 2", "blacklist": "False"},
        )
        BlacklistedUser.objects.create(
            username="test",
            uid="123123",
    ...
    def test_aoi_changesets_feed_view(self):
        ...
        self.assertEqual(response.status_code, 200)
        rss_data = list(list(ET.fromstring(response.content))[0])
        title = [i for i in rss_data if i.tag == "title"][0]
        items = [i for i in rss_data if i.tag == "item"]
        link = [i for i in list(items[0]) if i.tag == "link"][0]
        self.assertIn("?aoi=", link.text)
        self.assertEqual(
            title.text,
...
class TestAoIStatsAPIViews(APITestCase):
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            username="test_user", email="b@a.com", password="password", is_staff=True
        )
        UserSocialAuth.objects.create(
            user=self.user,
            provider="openstreetmap",
        ...
        self.reason_2 = SuspicionReasonsFactory(name="Mass Deletion", is_visible=False)
...
class TestBlacklistedUserListAPIView(APITestCase):
    def setUp(self):
        ...
        self.staff_user = User.objects.create_user(
            username="staff_user", email="b@a.com", password="password", is_staff=True
        )
        UserSocialAuth.objects.create(
...
class TestBlacklistedUserDetailAPIViews(APITestCase):
    def setUp(self):
        ...
        self.url = reverse("supervise:blacklist-detail", args=[self.blacklisted.uid])
    ...
    def test_normal_user_getting_staff_user_blacklist(self):
        ...
        self.client.login(username=self.user.username, password="password")
        response = self.client.get(reverse("supervise:blacklist-detail", args=[4999]))
