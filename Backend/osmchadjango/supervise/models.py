    ...
    def changesets(self, request=None):
        ...
        if "objectTags" in self.filters and "filterArea" in self.filters:
            ...
            if newObjectTagAreaGeoJSON is not None:
                ...
                qs = qs.filter(bbox__intersects=self.objectTagArea)
        ...
        if self.geometry is not None:
            qs = qs.filter(bbox__intersects=self.geometry)

        if (
            len(Changeset.objects.all()) == qs.count()
        ):  # This assumes that not all Changesets match the filter
            self.filterStatus["newChangesets"] = []
            self.save()
            return Changeset.objects.none()
